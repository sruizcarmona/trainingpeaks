...
ath.testdates <- readr::read_csv('../../2504_UPDATE/athlete_dates_2504.csv',
                                   col_types = cols("c",
...
all.dirs.PH <- list.dirs('../../2504_UPDATE/',recursive = F)
```

### Load previous results

```{r}
if(file.exists('out/rdas/tpeaks_newzones_cmr2504.rda')){
  load('out/rdas/tpeaks_newzones_cmr2504.rda')
}
```
...
for (athlete in sel.athletes){
  ...
  ## check if there are files, if not, skip
  files <- get_list_files(athlete)
  if (length(files) == 0){
    writeLines(c("No files found, skipping athlete."))
    next
  }
  ...
  if(!exists("tp.newzones")){
    ...
    tp.newzones <- data.frame(name=athlete, ath.id=paste0("TP25_",str_pad(1,3,pad="0")),
    ...
    if(!athlete %in% tp.newzones$name){
      ...
      tp.newzones$ath.id[tp.newzones$name == athlete] <- paste0("TP25_",str_pad(dim(tp.newzones)[1],3,pad="0"))
  ...
  save(tp.newzones,file='out/rdas/tpeaks_newzones_cmr2504.rda')
}
# stop cluster
stopCluster(cl)
# save everything in a file, so we don't need to calculate them every time
save(tp.newzones,file='out/rdas/tpeaks_newzones_cmr2504.rda')
# tp.newzones %>% arrange(name)
# ath.testdates %>% arrange(ATHLETE)

## Get only the XXX smallest files, for calculating the maxHR
# files <- file.info(get_list_files(athlete)) %>%
#   arrange(size) %>%
#   select(size) %>%
#   slice_head(n = 300) %>%
#   rownames()
...
```{r}
tpeaks.all <- NULL
tpeaks.all <- readRDS("tempfiles/tpeaks_all_1179_2024.rds")
###################
# initialize cluster
###################
# set up number of cores
...
for (athlete in sel.athletes){
  ...
  # check if athlete already processed
  if (file.exists(paste0('tempfiles/tpeaks_all_',athlete,'.rds'))){
    writeLines(paste0("Athlete ", athlete, " already processed, skipping..."))
    # tpeaks.all <- rbind(tpeaks.all,
    #                     readRDS(paste0('tempfiles/tpeaks_all_',athlete,'.rds')))
    next
  } else if (athlete %in% tp.newzones$name && tp.newzones$maxHR[tp.newzones$name == athlete] == 999){
    writeLines(paste0("Athlete ", athlete, " has been marked for manual processing, skipping..."))
    next
  } else {
    writeLines(paste0("Athlete ", athlete, " not processed yet, calculating..."))
  }
