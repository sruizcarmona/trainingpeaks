...
ath.testdates <- readr::read_csv('../../2210_TRAININGPEAKS/athletes_MH_test_2210.csv',
                                   col_types = cols("c",
                                                    col_date(format="%d/%m/%Y")))
ath.testdates2 <- readr::read_csv('../../2210_TRAININGPEAKS/athletes_PH_test_2210.csv',
                                   col_types = cols("c",
                                                    col_date(format="%d/%m/%Y")))
...
# sel.athletes <- unique(sel.athletes)[1:10]
...
hr_summary <- all.activities %>%
  ...
  mutate(hr.z1.perc.total = round(hr.z1.min.total / duration.min.total * 100, 1),
...
hr_summary <- hr_summary %>% 
  ...
  mutate(duration.min.total.noHR = ifelse(is.na(duration.min.total.noHR), 0, duration.min.total.noHR),
         perc.total.withHR = duration.min.total.withHR / 
           (duration.min.total.withHR + duration.min.total.noHR) * 100,
         perc.total.withHR = round(perc.total.withHR, 1)) %>% 
  relocate(duration.min.total.noHR, .after=duration.min.total.withHR) %>% 
  relocate(perc.total.withHR, .after=duration.min.total.noHR)
