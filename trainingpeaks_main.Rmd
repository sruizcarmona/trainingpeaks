...
dup.sessions <- tpeaks.all %>%
  ...
  mutate(filename=sapply(str_split(file,"/"),function(x) x[8])) %>%
...
write.csv(ath.info, file=paste0("out/csv/tpeaks_summary_athletes_",format(as.Date(Sys.Date()),format="%y%m%d"),".csv"))
