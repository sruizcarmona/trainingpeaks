...
process_maxhr_activity <- function(athid, myact, w_e, w_d, w_x, w_h, nlag = 10, d2f = 0.1) {
  myres <- ggplot_build(ggplot(myact) + stat_density(aes(x=hrmax),bw=3))$data[[1]] %>% 
    ...
    mutate(rank_error = dense_rank(error_act * error_perc), 
           ...
           score_error = as.vector(scale(error_act * (error_perc5))) + abs(min(scale(error_act * (error_perc5)), na.rm=T)),
           score_error_perc = as.vector(scale(error_perc5)) + abs(min(scale(error_perc5), na.rm=T)),
           rank_deriv = dense_rank(desc(deriv)),
           # same with derivative, no point on ranking, better to leave it scaled as it is
           score_deriv = as.vector(scale(-deriv)),
  ...
  myres <- myres %>% 
    ...
    ungroup() %>% 
    mutate(optim = (-score_error * w_e + score_deriv * w_d + (highness ** 2) * w_h) * score_deriv2,
           optim = ifelse(optim==0, -100, optim))
  return(myres)
}
...
for (myfile in optim_files) {
  ...
  sourcedata <- get(varname) %>% filter(hrmax > 1) %>% 
  ...
  maxhr_allact <- get_maxhr(pdata)
  
  # adapt mylim_x
  if(dim(sourcedata)[1] < 10 | pdata$x[which.min(pdata$optim)] < 150) {mylim_x <- 89} else {mylim_x <- 139}
  # save plot
  png(filename=paste0('png/wrist_vs_chest_',athid,'.png'),width = 2800, height = 1440,res=300)
  ...
  p <- plot_maxtangent(sourcedata, pdata, athid, mylim_x, dev_info, chest_perc, col="black") + 
    labs(subtitle = "All activities")
  
  # check random splits for comparing with wrist/chest (not for plotting)
  ...
  if (chest_perc == 100 | chest_perc == 0) {
    ...
    split_1 <- sourcedata %>% filter(row_number() %in% split_rows)
    split_2 <- sourcedata %>% filter(!row_number() %in% split_rows)
    split_col <- ifelse(chest_perc == 100, "bisque", "aquamarine")
    ...
    if (dim(chest_only)[1] < 10) {
      ...
      maxhr_chest <- get_maxhr(chest_only_pdata)
    ...
    if (dim(wrist_only)[1] < 10) {
      ...
      maxhr_wrist <- get_maxhr(wrist_only_pdata)
  ...
  dev.off()
  # all_maxhr_athletes <- rbind(all_maxhr_athletes,
  #                             c(athid, maxhr_allact, maxhr_split1, maxhr_split2, maxhr_chest, maxhr_wrist, chest_perc))
}
```
...
```{r}
all_maxhr_athletes <- setNames(type.convert(data.frame(all_maxhr_athletes)),
                               c("ath.id", "all", "split1", "split2", "chest", "wrist", "chest_perc"))
all_maxhr_athletes[59,"split2"] <- 180
all_maxhr_athletes
```

```{r}
wdata <- all_maxhr_athletes %>%
  # drop_na() %>%
  mutate(chest_perc = round(chest_perc,1),
         wrist_perc = 100 - chest_perc,
         chestdiff = chest - all,
         wristdiff = wrist - all, 
         splitdiff = split2 - split1,
         wristchestdiff = wrist - chest)
wdata
```

```{r}
wdata_long <- wdata %>% 
  pivot_longer(cols= c("splitdiff", "wristchestdiff",
                       # "chestdiff",
                       # "wristdiff"
                       )) %>% 
  select(name, value) %>% 
  ...
  mutate(absvalue = abs(value),
         nameok = ifelse(name == "splitdiff", "Random splits difference", "Wrist vs chest difference"))
wdata_long 
```


```{r}
ggplot(wdata_long, aes(x=name, y=value, fill=name)) +
  geom_point() +
  geom_violin(width=1.1) +
  geom_boxplot(width=0.1, color="grey", show.legend = F) 
```

```{r}
# wdata_long
t.test(wdata_long %>% filter(name == "splitdiff") %>% pull(value),
       wdata_long %>% filter(name == "wristchestdiff") %>% pull(value))
```
...
```{r}
library(ggstatsplot)
ggbetweenstats(wdata_long,
               x=nameok,
               y=value,
               title = "Differences between chest and wrist-based monitors",
               ylab = "Difference (bpm)",
               xlab = "Comparisons",
               ...
               package          = "ggsci",
              results.subtitle = T, bf.message = F
                )
```
